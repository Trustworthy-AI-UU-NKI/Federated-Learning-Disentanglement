"""
Implementation from https://github.com/spthermo/SDNet/blob/master/layers/blocks.py
"""
...
def upconv(in_channels, out_channels, norm="batchnorm"):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, 3, 1, 1), normalize(out_channels, norm)
    )


def conv_block_unet(
    in_channels, out_channels, kernel_size, stride=1, padding=0, norm="batchnorm"
):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding),
        normalize(out_channels, norm),
...
class ResConv(nn.Module):
    def __init__(self, ndf, norm):
        ...
        self.conv2 = conv_preactivation_relu(
            self.ndf * 2, self.ndf * 2, 3, 1, 1, self.norm
        )
        self.resconv = conv_preactivation_relu(
            self.ndf, self.ndf * 2, 1, 1, 0, self.norm
        )
